...
- `[G-1]` Analyze every sidecar subtitle file once per full file path and scan version, except that a current-version `sync_requested` file is rechecked on later eligible passes; never re-analyze a file that already `passed` or was `forced_removed` at the current scan version.
...
| `[KD-3.1]` Registry scope       | Retain one latest analyzed record per `filePath`, with its scan version and verdict (`passed`, `forced_removed`, `sync_requested`); at the current version, skip `passed` and `forced_removed` but recheck `sync_requested`. | A `sync_requested` row records the request without suppressing a later validity check; retaining it while still invalid avoids repeating the request. A successful re-analysis at a changed version replaces the path's prior row, so no historical versions are retained. Identical contents at different paths are independent candidates, not one global verdict. |
| `[KD-4]` Unwanted forced        | An unregistered `.<lang>.srt` file (no `.forced.` marker) satisfying the forced heuristic is deleted through Bazarr; a `sync_requested` recheck alerts instead.                                                              | A forced track in the full-subtitle slot hides the real gap from Bazarr; deleting through Bazarr rather than the filesystem updates its history so the language returns to the wanted list and is searched again. Files named `.forced.srt` are wanted by construction. Replacement search is left to Bazarr's wanted-search schedule.                               |
| `[KD-5]` Out-of-sync detection  | Match sorted start timestamps one-to-one within 500 ms between sibling `.srt` files; a pair is divergent when fewer than half the shorter track's cues match.                                                                | Translations split or omit cues, so cue indices are not correspondence. Time-based matching avoids Anora's false positive; 500 ms tolerance reduces sensitivity to small timing differences; sibling agreement remains a heuristic, not proof of audio alignment.                                                                                                    |
| `[KD-6.1]` Sync target          | In a divergent pair, request Bazarr sync only for unregistered files; current-version `sync_requested` files are rechecked but never synced again, and `passed` files are skipped.                                           | A file that already passed against an earlier sibling is the more trustworthy side, while a prior sync request must not repeat. Bazarr's sync aligns against audio, so syncing both when neither file is registered converges regardless of which drifted.                                                                                                           |
...
- `[C-4]` Same-path rewrites and replacements reuse a current-version `passed` or `forced_removed` verdict. A current-version `sync_requested` path is instead rechecked on each eligible later pass using the same forced and sibling timing heuristics; it retains that verdict while invalid, so it is never synced twice. Renaming or bumping the scan version makes any path eligible as unregistered.
...
The feature owns `SUBTITLE_SCAN_VERSION = 2`, a positive integer constant shared by `/subtitlescan` and the scheduled pass. Increment it when file-analysis changes require invalidating prior verdicts. Raising the timing tolerance from 300 ms to 500 ms preserves prior passing verdicts and does not bump the version; the 50% match requirement and forced-detection thresholds are unchanged. Current-version `sync_requested` paths are already rechecked with the relaxed tolerance. The next pass treats all paths without a verdict at that exact version as unregistered, including paths with older `passed`, `forced_removed`, or `sync_requested` verdicts. Old verdicts are neither skip signals nor trusted sync references. A bump does not launch a pass itself; the schedule or command does.
...
Passing records survive restarts and have no time-based expiry. Scheduled and manual passes use the same registry: a `passed` or `forced_removed` path is never analyzed or actioned again at that scan version, though a passed path's contents may be read as a reference for a candidate sibling (`[KD-6.1]`). A `sync_requested` path is rechecked on each eligible later pass; when valid it is replaced with `passed`, and when still invalid it retains `sync_requested`. A rename or scan-version bump is a new candidate; a content rewrite at a passed or forced-removed path is not. Failed or interrupted analysis never records `passed`, and a failed registry write leaves the path eligible for a later pass.
...
for each candidate:
  if isForcedSubtitle(path, duration):
    if known.verdict = sync_requested → alertInvalid(path); retain sync_requested # no delete or sync
    else → bazarr.deleteSubtitle(item, lang); record(path, version, forced_removed)
remaining ← candidates not invalid or removed by the forced check
for each pair (a, b) of non-forced sidecars with at least one in remaining:
  if divergent(a, b):
    for each current-version sync_requested candidate in the pair and remaining: alertInvalid(path); retain sync_requested # no delete or sync
    for each unregistered member in the pair and remaining: bazarr.syncSubtitle(item, lang); record(path, version, sync_requested)
record(path, version, passed) for every remaining candidate with no divergent pair
```

`isForcedSubtitleContent`, `parseStartTimestamps`, and the divergence rule are shared helpers: fewer than 3 cues per minute or under 15% screen-time ratio for forced; fewer than 50% of the shorter track's starts matched within 500 ms for divergence. Both parsers handle LF and CRLF cue separators. Matching sorts timestamps and advances through both tracks, consumes each matched cue once, and skips unmatched starts instead of shifting all subsequent comparisons. An empty track provides no timing evidence and does not trigger sync. The Bazarr item is resolved once per media by path (`getMovieByPath` or `getEpisodeByPath`); an unresolvable item logs a warning and records nothing, so the media is retried next pass. A file that is deleted, newly sync-requested, or still-invalid after a sync request is never recorded as `passed` in the same pass.

A current-version `sync_requested` candidate is rechecked with the same forced and sibling timing heuristics. If either marks it invalid, send one plain-text message to `TELEGRAM_CHAT_ID`: `Invalid subtitle for <media title> (<language>)`; retain `sync_requested` and neither delete nor sync it. A valid rechecked candidate is recorded as `passed`. Invalid alerts repeat on every eligible later recheck while the file remains invalid. Newly divergent candidates still receive a Bazarr sync request and `sync_requested` record, but receive no Telegram sync notification. Successful Bazarr sync and delete requests, passing records, and invalid alerts are logged with diagnostics; existing failure logging is retained. Telegram failures are logged without changing the verdict or stopping other candidates; alerts are best-effort and do not prevent later eligible rechecks.
...
- `[SO-3]` A current-version `sync_requested` subtitle is rechecked without a version bump: an invalid recheck alerts Telegram and retains that verdict without a delete or repeat sync, while a valid recheck passes.
- `[VC-4]` Service tests verify the configured recipient, exact invalid-alert media title and subtitle language, absence of a file path, no Telegram notification for newly requested syncs, repeated invalid alerts on eligible later passes, no delete/re-sync of an invalid recheck, passing of a valid recheck, and continued scanning with retained verdicts after Telegram failure — demonstrates `[SO-3]`.
- `[SO-5]` `passed` and `forced_removed` subtitle paths skip hashing and unnecessary content reads across passes and restarts, including same-path replacements; current-version `sync_requested` paths are the deliberate recheck exception — demonstrated by `[VC-6]` and `[VC-7]`.
- `[VC-6]` Discovery and service tests verify no contents are read when every path is `passed` or `forced_removed`, renamed paths and identical contents at different paths are independently eligible, version changes recheck files, `sync_requested` paths use the forced and sibling timing heuristics again, and passed siblings remain trusted sync references — demonstrates `[SO-5]`.
