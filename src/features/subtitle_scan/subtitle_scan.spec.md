...
After a successful Bazarr sync request and `sync_requested` registry write, send one plain-text message to `TELEGRAM_CHAT_ID`: `Subtitle resync requested for <media title> (<language>)`. This reports a request, not verified subtitle alignment. The notification intentionally excludes the subtitle file path; diagnostic logs retain it. Failed or unresolved sync requests and cached paths produce no notification. Telegram failures are logged without undoing the verdict or stopping other targets; notifications are best-effort and are not retried on later passes. Notification-only changes do not increment `SUBTITLE_SCAN_VERSION` (still 2) or replay existing results.
...
- `[VC-4]` Service tests verify the configured recipient, media title, subtitle language, absence of a file path, one notification per sync target, silence for cached/failed requests, and continued scanning with retained verdicts after Telegram failure — demonstrates `[SO-3]`.
