...
- `[G-1]` Analyze every sidecar subtitle file once per full file path and scan version, except that a current-version `sync_requested` file is rechecked on later eligible passes; never re-analyze a file already marked `passed`, `forced_removed`, or `invalid` at the current scan version.
...
| `[KD-6.1]` Sync target          | In a divergent pair, request Bazarr sync only for unregistered files; current-version `sync_requested` files are rechecked but never synced again, and `passed` or `invalid` files are skipped.                             | A file that already passed against an earlier sibling is the more trustworthy side, while a prior sync request must not repeat. Bazarr's sync aligns against audio, so syncing both when neither file is registered converges regardless of which drifted.                                                                                                                                  |
...
- `[C-4]` Same-path rewrites and replacements reuse a current-version `passed`, `forced_removed`, or `invalid` verdict. A current-version `sync_requested` path is instead rechecked on the next eligible pass using the same forced and sibling timing heuristics; a failed recheck records terminal `invalid`, so it is never synced twice. Renaming or bumping the scan version makes any path eligible as unregistered.
...
Terminal records survive restarts and have no time-based expiry. Scheduled and manual passes use the same registry: a `passed`, `forced_removed`, or `invalid` path is never analyzed or actioned again at that scan version, though a passed path's contents may be read as a reference for a candidate sibling (`[KD-6.1]`). Invalid paths are never references. A `sync_requested` path is rechecked on the next eligible pass; when valid it is replaced with `passed`, and when still invalid it becomes `invalid`. A rename or scan-version bump is a new candidate; a content rewrite at a terminal path is not. Failed or interrupted analysis never records `passed`, and a failed registry write leaves the path eligible for a later pass.

`migrations/20260923115940_invalid_subtitle_verdict` adds `invalid` to the PostgreSQL enum without changing existing records. No scan-version bump is needed: existing `sync_requested` paths transition on their next eligible recheck.
...
for each pair (a, b) of non-forced sidecars with at least one in remaining:
  if divergent(a, b):
    for each current-version sync_requested candidate in the pair and remaining: record(path, version, invalid); alertInvalid(path) # no delete or sync
...
A current-version `sync_requested` candidate is rechecked with the same forced and sibling timing heuristics. If either marks it invalid, record `invalid` and send one best-effort plain-text message to `TELEGRAM_CHAT_ID`: `Invalid subtitle for <media title> (<language>)`; neither delete nor sync it. A valid rechecked candidate is recorded as `passed`. Invalid paths skip future scans and alerts at the current version. Newly divergent candidates still receive a Bazarr sync request and `sync_requested` record, but receive no Telegram sync notification. Successful Bazarr sync and delete requests, passing records, and invalid alerts are logged with diagnostics; existing failure logging is retained. Telegram failures are logged without changing the persisted `invalid` verdict or stopping other candidates; failed alerts are not retried.
...
- `[SO-3]` A current-version `sync_requested` subtitle is rechecked without a version bump: an invalid recheck records terminal `invalid` and alerts Telegram without a delete or repeat sync, while a valid recheck passes.
- `[VC-4]` Service tests verify the configured recipient, exact invalid-alert media title and subtitle language, absence of a file path, no Telegram notification for newly requested syncs, skipping invalid paths without content reads, probes, Bazarr lookups, scan-budget consumption, or repeated alerts, exclusion of invalid paths from sibling references, no delete/re-sync of an invalid recheck, passing of a valid recheck, and continued scanning with persisted `invalid` verdicts after Telegram failure — demonstrates `[SO-3]`.
- `[SO-5]` `passed`, `forced_removed`, and `invalid` subtitle paths skip hashing and unnecessary content reads across passes and restarts, including same-path replacements; current-version `sync_requested` paths are the deliberate recheck exception — demonstrated by `[VC-6]` and `[VC-7]`.
- `[VC-6]` Discovery and service tests verify no contents are read when every path is `passed`, `forced_removed`, or `invalid`, renamed paths and identical contents at different paths are independently eligible, version changes recheck files, `sync_requested` paths use the forced and sibling timing heuristics again, and passed siblings remain trusted sync references — demonstrates `[SO-5]`.
