...
const liveReplacementOperations: ReplacementOperations = { copyFile, exists: safeExistsSync, fsync, remove: rm, rename }

const stageOperation = <Success>(filePath: string, operation: string, run: () => Promise<Success>) =>
...
export const replaceOutputs = (
  ...
  const stage = Effect.gen(function* () {
    for (const output of outputs) {
      yield* stageOperation(output.stage, 'stage replacement', () => operations.copyFile(output.source, output.stage))
      yield* stageOperation(output.stage, 'fsync replacement stage', () => operations.fsync(output.stage))
    }
    yield* stageOperation(inputDirectory, 'fsync replacement directory', () => operations.fsync(inputDirectory))
  })
...
export const handlePostTranscode = (params: { filePath: string; mediaTitle: string; mediaType: 'movie' | 'show' }) =>
  Effect.gen(function* () {
    ...
    for (const error of installation.cleanupErrors) {
      yield* Effect.sync(() => logError(error, 'postTranscode', params.mediaTitle))
    }
    ...
    if (params.mediaType === 'movie') {
      ...
      if (movieId === undefined) {
        yield* Effect.sync(() => logger.warn(`Could not find movie in Radarr for path: ${params.filePath}`, 'postTranscode', params.mediaTitle))
        return
      }
      ...
      if (seriesId === undefined) {
        yield* Effect.sync(() => logger.warn(`Could not find series in Sonarr for path: ${params.filePath}`, 'postTranscode', params.mediaTitle))
        return
      }
