...
Autoscan is organised by concern: cross-cutting wiring (`core/`), runtime providers (`providers/`), shared primitives
(`shared/`), external API clients (`integrations/`), cross-feature business domains (`domains/`), and
business-capability features (`features/`). This spec pins the layout so that contributors (human or agent) know where
new code goes and when an import crosses a boundary it should not.
...
| `[KD-2]` Feature isolation    | Each feature owns everything it needs under one folder and exposes a declarative `feature.ts` consumed by `registerFeatures` (see `docs/architecture/feature_registration.spec.md`)                                                                                            | A feature is independently deletable; the wiring surface is one declaration                               |
...
| `[KD-5]` Domains layer        | `src/domains/<name>/` holds cross-feature business domains (services, repositories, errors, types); same kind-subfolder rule as features; no `feature.ts`; co-located `*.spec.md`                                                                                              | Dedicated home for business logic shared across features; `shared/` stays utility-only                    |
...
- `[PI-4]` **Kind suffix ↔ subfolder.** `*.service.ts` lives in `services/`, `*.command.ts` in `commands/`, etc.
  The only feature-root files are `feature.ts`, `errors.ts`, `types.ts`, and the spec.
- `[PI-5]` **`feature.ts` is the feature's wiring declaration.** `src/features/index.ts` aggregates every
  `<name>Feature` into a `features` array and `core/bootstrap.ts` calls `registerFeatures(features)` once; nothing
  else in a feature is imported by core.
...
| `src/features/`     | Business capabilities, each independent with a `feature.ts` (dynamic_dns, language_sync, queue_cleanup, send_message, trakt_sync, transcoding) |
...
| Feature independence         | `src/features/<f>/`                                                                                        | `feature.ts` (only wiring entry) + `[VC-1]`           |
| Domain independence          | `src/domains/<d>/`                                                                                         | No `feature.ts`; passive exports + `[VC-8]`           |
| Bootstrap wiring             | `src/core/bootstrap.ts`                                                                                    | Calls `registerFeatures(features)` once — `[VC-5]`    |
...
- `[VC-3]` Every feature folder contains a `feature.ts` exporting a `<name>Feature` constant typed as `Feature`.
- `[VC-4]` Every feature, every provider, and every domain has a co-located `*.spec.md`.
- `[VC-5]` `core/bootstrap.ts` imports `{ features }` from `src/features/index.ts` and invokes
  `registerFeatures(features)` exactly once.
