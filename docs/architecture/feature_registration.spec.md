...
The registry explicitly imports the language-sync, plex-auth, queue-cleanup, send-message, and transcoding declarations and exposes them in their registration order (`src/features/index.ts:1`). `registerFeatures` iterates that array and forwards each present category to the corresponding provider, skipping absent collections (`src/core/feature.ts:43`).
