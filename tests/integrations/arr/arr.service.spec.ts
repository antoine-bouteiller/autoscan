...
describe('ArrClient queue pagination', () => {
  test('requests one authenticated page when it contains the full queue', async () => {
    const fetchMock = spyOn(globalThis, 'fetch').mockResolvedValue(Response.json({ records: [makeItem(1)], totalRecords: 1 }))

    const queue = await Effect.runPromise(makeClient().getQueue())
  ...
  test('aggregates every queue page in order', async () => {
    const pages = [
  ...
  test('stops on an empty page when the reported total is inconsistent', async () => {
    const pages = [
  ...
  test('fails rather than returning partial data when a later page fails', async () => {
    ...
    const exit = await Effect.runPromiseExit(makeClient().getQueue())
