...
yield * effect.pipe(Effect.catchAll((err) => Effect.fail(new InternalServerError({ message: 'Something failed' }))))
...
const withRetry = <A, E extends { retryable?: boolean }, R>(effect: Effect.Effect<A, E, R>): Effect.Effect<A, E, R> =>
  effect.pipe(
    Effect.retry(
