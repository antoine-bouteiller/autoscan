...
export class OrderService extends Effect.Service<OrderService>()('OrderService', {
  accessors: true,
  dependencies: [UserService.Default, ProductService.Default, InventoryService.Default, PaymentService.Default],
...
const ServicesLive = Layer.mergeAll(UserService.Default, OrderService.Default, ProductService.Default, NotificationService.Default)
...
const AppLive = UserService.Default.pipe(
  Layer.provide(OrderService.Default.pipe(Layer.provide(ProductService.Default.pipe(Layer.provide(DatabaseLive)))))
)
```
...
const AppLive = Layer1.pipe(Layer.provide(Layer2.pipe(Layer.provide(Layer3.pipe(Layer.provide(Layer4.pipe(Layer.provide(Layer5))))))))
...
const AppLive = Layer.mergeAll(Layer1, Layer2).pipe(Layer.provide(InfraLive))
...
export class ElectricEventQueue extends Effect.Service<ElectricEventQueue>()('ElectricEventQueue', {
  ...
  static readonly layerConfig = (config: Config.Config.Wrap<EventQueueConfig>): Layer.Layer<ElectricEventQueue, ConfigError.ConfigError> =>
    Layer.unwrapEffect(Config.unwrap(config).pipe(Effect.map((cfg) => Layer.succeed(ElectricEventQueue, new ElectricEventQueueImpl(cfg)))))
}

// Usage
const EventQueueLive = ElectricEventQueue.layerConfig({
  maxRetries: Config.integer('EVENT_QUEUE_MAX_RETRIES').pipe(Config.withDefault(3)),
  batchSize: Config.integer('EVENT_QUEUE_BATCH_SIZE').pipe(Config.withDefault(100)),
  pollInterval: Config.integer('EVENT_QUEUE_POLL_INTERVAL').pipe(Config.withDefault(1000)),
})
```
...
export const TestLive = Layer.mergeAll(UserServiceTest, OrderServiceTest, ProductServiceTest).pipe(Layer.provide(InMemoryDatabaseLive))
...
const LoggerLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const transport = config.env === 'production' ? createCloudTransport() : createConsoleTransport()
