...
const processPayment = Effect.fn('PaymentService.processPayment')(function* (orderId: OrderId, amount: number) {
  // ...
})
```
...
const LoggerLive = Layer.unwrapEffect(
  Effect.gen(function* () {
    const level = yield* Config.literal('debug', 'info', 'warning', 'error')('LOG_LEVEL').pipe(Config.withDefault('info'))
