...
yield *
  repo.findById(id).pipe(
    Effect.catchTag('DatabaseError', (err) => Effect.fail(new UserNotFoundError({ userId: id, message: 'Lookup failed' }))),
    Effect.catchTag('ConnectionError', (err) => Effect.fail(new ServiceUnavailableError({ message: 'Database unreachable' })))
  )
...
const MainLive = DatabaseLive.pipe(Layer.provideMerge(ConfigServiceLive), Layer.provideMerge(LoggerLive), Layer.provideMerge(CacheLive))
```
...
const config = Config.all({
  ...
  maxRetries: Config.integer('MAX_RETRIES').pipe(Config.validate({ message: 'Must be positive', validation: (n) => n > 0 })),
})
```
